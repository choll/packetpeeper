...
int main(void)
{
	...
	if((timeout = malloc(sizeof(*timeout))) == NULL) {
		[[ErrorStack sharedErrorStack] pushError:@"Failed to allocate memory for select(2) timeout data" lookup:[PosixError class] code:errno severity:ERRS_ERROR];
		err_exit(objio);
	}
	...
	bpf_fd = [bpf fd];	/* the bpf file descriptor is needed for use with select */

	if(chrootuser(CHROOT_DIR, USER) == -1) {
		[[ErrorStack sharedErrorStack] pushError:@"Failed to drop privileges" lookup:[PosixError class] code:errno severity:ERRS_ERROR];
		err_exit(objio);
	}
		...
		if(FD_ISSET(sock_fd, &rset)) {
			do {
				...
				if([obj isMemberOfClass:[MsgSettings class]]) {
					...
					[[ErrorStack sharedErrorStack] pushError:@"Unknown object recieved by helper" lookup:Nil code:0 severity:ERRS_ERROR];
					err_exit(objio);
				}
				[obj release];
		...
		if(FD_ISSET(bpf_fd, &rset)) {
			...
			if((parray = [bpf read]) == nil) {
				...
				for(i = 0; i < [parray count]; ++i) {
					...
					if((tempData = [[NSData alloc] initWithBytesNoCopy:(uint8_t *)[data bytes] + hdr->bh_hdrlen
...
int init_objio(id *obj, const char *path)
{
	...
	if(strlcpy(s_addr.sun_path, path, sizeof(s_addr.sun_path)) >= sizeof(s_addr.sun_path))
		return -1;
