...
enum endian_modifier {NATIVE_ORDER_NATIVE_ALIGN,
...
static size_t pp_unpack_parse_endian_modifier(const char *format, enum endian_modifier *endian_modifier);
static size_t pp_unpack_parse_repeat_modifier(const char *format, unsigned int *repeat_modifier);
static size_t pp_unpack_parse_type(const char *format, enum data_type *data_type);
...
static PyMethodDef ppMethods[] = {{"size", pp_size, METH_VARARGS, "Number of bytes in target data"},
...
- (id)init
{
	if((self = [super init]) != nil) {
		if(!Py_IsInitialized()) {
            ...
            const char* temp =
                [[NSString stringWithFormat:@"import sys\nsys.path.append(\"%@\")\n",
                    [[NSBundle mainBundle] builtInPlugInsPath]] UTF8String];
...
- (BOOL)loadModule:(NSString *)moduleName
{
	...
	struct {
		...
		{"canDecodeProtocol", &canDecodeProtocolFunc},
	...
	for(i = 0; i < sizeof(funcs) / sizeof(funcs[0]); ++i) {
		if(!PyObject_HasAttrString(module, (char *)funcs[i].funcName)) {
			NSLog(@"Function '%@.%s' is missing", moduleName, funcs[i].funcName);
			continue;
		}
		if((*funcs[i].funcObject = PyObject_GetAttrString(module, (char *)funcs[i].funcName)) == NULL) {
			NSLog(@"Failed to load function '%@.%s'", moduleName, funcs[i].funcName);
			continue;
		}
		if(!PyCallable_Check(*funcs[i].funcObject)) {
			NSLog(@"Symbol '%@.%s' is not callable", moduleName, funcs[i].funcName);
			continue;
		}
	}
...
- (NSArray *)columnIdentifiers
{
	...
	if(value != NULL && PyList_Check(value)) {
		...
		for(i = 0; i < PyList_Size(value); ++i) {
			...
			if(py_longName == NULL || py_shortName == NULL || !PyString_Check(py_longName) || !PyString_Check(py_shortName))
				continue;
			...
			column = [[ColumnIdentifier alloc] initWithPlugin:self index:i longName:longName shortName:shortName];
...
static PyMODINIT_FUNC init_packetpeeper_module(void)
{
void *Py_InitModule4TraceRefs(char *name, void *methods,
                                            char *doc, void *self,
                                            int apiver);
...
static PyObject *pp_unpack(PyObject *self, PyObject *args)
{
	...
	while(format[i] != '\0') {
		...
		if(data_type == TYPE_STRING) {
			...
			snprintf(temp, repeat_modifier + 1, "%.*s", repeat_modifier, (char *)pkt_data);
			...
			while(repeat_modifier-- > 0) {
				...
				switch(data_type) {
					...
					case TYPE_UINT32:
						...
						if(endian_modifier == BIG_ORDER_STD_ALIGN)
							item = PyLong_FromUnsignedLong(be32toh(*(uint32_t *)pkt_data));
						else if(endian_modifier == LITTLE_ORDER_STD_ALIGN)
							item = PyLong_FromUnsignedLong(le32toh(*(uint32_t *)pkt_data));
						else
					...
					case TYPE_INT64:
						...
						if(endian_modifier == BIG_ORDER_STD_ALIGN)
							item = PyLong_FromLongLong(be64toh(*(int64_t *)pkt_data));
						else if(endian_modifier == LITTLE_ORDER_STD_ALIGN)
							item = PyLong_FromLongLong(le64toh(*(int64_t *)pkt_data));
						else
					...
					case TYPE_UINT64:
						...
						if(endian_modifier == BIG_ORDER_STD_ALIGN)
							item = PyLong_FromUnsignedLongLong(be64toh(*(uint64_t *)pkt_data));
						else if(endian_modifier == LITTLE_ORDER_STD_ALIGN)
							item = PyLong_FromUnsignedLongLong(le64toh(*(uint64_t *)pkt_data));
						else
							item = PyLong_FromUnsignedLongLong(*(uint64_t *)pkt_data);
						break;
					...
					default: /* this can never be reached, only really here to suppress compiler warnings */
						PyErr_SetString(PyExc_TypeError, "invalid format string");
						goto err;
				}
