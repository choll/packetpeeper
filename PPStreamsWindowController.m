...
+ (NSMenu *)createStreamTableMenu
{
	...
	NSMenuItem *item;
	unsigned int i;
	NSString *representedObjects[] = {PPSTREAMSWINDOW_STREAMS_TABLE_SRC_IP_ADDRESS, PPSTREAMSWINDOW_STREAMS_TABLE_DST_IP_ADDRESS,
									  PPSTREAMSWINDOW_STREAMS_TABLE_SRC_HOSTNAME, PPSTREAMSWINDOW_STREAMS_TABLE_DST_HOSTNAME,
									  PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORT, PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORT,
									  PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORTNAME, PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORTNAME,
									  PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_SENT, PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_RECV,
									  PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_TOTAL, PPSTREAMSWINDOW_STREAMS_TABLE_STATUS};
	NSString *titles[] = {@"Source IP Address", @"Destination IP Address", @"Source Hostname", @"Destination Hostname",
						  @"Source Port", @"Destination Port", @"Source Port Name", @"Destination Port Name",
						  @"Bytes Sent", @"Bytes Received", @"Bytes Total", @"Status"};
...
- (void)windowDidLoad
{
	autoScrolling = [[NSUserDefaults standardUserDefaults] boolForKey:PPSTREAMSWINDOW_AUTOSCROLLING];
	...
	[streamTableView setColumnAutoresizingStyle:NSTableViewNoColumnAutoresizing];
	[packetTableView setColumnAutoresizingStyle:NSTableViewNoColumnAutoresizing];

	[[packetTableView headerView] setMenu:[(AppController*)[NSApp delegate] createTCPProtocolsMenu]];
	[[streamTableView headerView] setMenu:[PPStreamsWindowController createStreamTableMenu]];

	[self populateStreamTableView];
	[self populatePacketTableView];

	syncMenu([self packetTableColumnMenu], [self packetTableColumnIdentifierStrings]);
	syncMenu([self streamTableColumnMenu], [self streamTableColumnIdentifierStrings]);
...
- (void)populatePacketTableView
{
	...
	if((arrayAsData = [[NSUserDefaults standardUserDefaults] objectForKey:PPSTREAMSWINDOW_PACKETTABLEVIEW_COLUMNS_KEY]) == nil)
		return;
...
- (void)populateStreamTableView
{
	...
	if((arrayAsData = [[NSUserDefaults standardUserDefaults] objectForKey:PPSTREAMSWINDOW_STREAMTABLEVIEW_COLUMNS_KEY]) == nil)
		return;
...
- (void)doubleAction:(id)sender
{
	...
	if(sender == packetTableView)
		[[self document] displayIndividualWindow:[selectedStream packetAtIndex:row]];

	if(sender == streamTableView)
		[[self document] displayReassemblyWindowForPacket:[[[[self document] tcpStreamController] streamAtIndex:row] packetAtIndex:0]];
}
...
- (IBAction)columnMenuAction:(id)sender
{
	...
	} else if([tableView numberOfColumns] > 1) { /* the user must have at least one column */
		[tableView removeTableColumn:[tableView tableColumnWithIdentifier:[sender representedObject]]];
...
- (void)savePacketTableViewColumns
{
	...
	[[NSUserDefaults standardUserDefaults] setObject:data forKey:PPSTREAMSWINDOW_PACKETTABLEVIEW_COLUMNS_KEY];
}
...
- (void)saveStreamTableViewColumns
{
	...
	data = [NSArchiver archivedDataWithRootObject:[streamTableView tableColumns]];

	[[NSUserDefaults standardUserDefaults] setObject:data forKey:PPSTREAMSWINDOW_STREAMTABLEVIEW_COLUMNS_KEY];
}
...
{
	return [self tableColumnIdentifierStringsForTableColumns:[packetTableView tableColumns]];
}

- (NSMutableArray *)streamTableColumnIdentifierStrings
{
	return [self tableColumnIdentifierStringsForTableColumns:[streamTableView tableColumns]];
}
...
#pragma clang diagnostic ignored "-Wincompatible-pointer-types"
	return [[selectedStream packetAtIndex:rowIndex] stringForColumn:[tableColumn identifier]];
...
- (id)streamTableObjectValueForTableColumn:(NSTableColumn *)tableColumn row:(int)rowIndex
{
	...
	if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_IP_ADDRESS]) {
		return [stream addrFrom];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_IP_ADDRESS]) {
		return [stream addrTo];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_HOSTNAME]) {
		return [stream hostFrom];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_HOSTNAME]) {
		return [stream hostTo];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORT]) {
		return [NSString stringWithFormat:@"%u", [stream srcPort]];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORT]) {
		return [NSString stringWithFormat:@"%u", [stream dstPort]];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORTNAME]) {
		return [stream srcPortName];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORTNAME]) {
		return [stream dstPortName];
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_SENT]) {
		return data_quantity_str([stream bytesSent]);
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_RECV]) {
		return data_quantity_str([stream bytesReceived]);
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_TOTAL]) {
		return data_quantity_str([stream totalBytes]);
	} else if([[tableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_STATUS]) {
...
- (void)streamTableDidClickTableColumn:(NSTableColumn *)tableColumn
{
	...
	if(lastStreamTableColumn == tableColumn) {
		...
		[[[self document] tcpStreamController] setReversePacketOrder:![[[self document] tcpStreamController] isReverseOrder]];
	} else {
		...
		if(lastStreamTableColumn != nil) {
			[streamTableView setIndicatorImage:nil inTableColumn:lastStreamTableColumn];
		...
		if((streams = [[NSMutableArray alloc] init]) != nil) {
			...
			while((n = [indexSet getIndexes:indexes maxCount:(sizeof(indexes) / sizeof(indexes[0])) inIndexRange:&range]) > 0) {
				for(i = 0; i < n; ++i)
					[streams addObject:[[[self document] tcpStreamController] streamAtIndex:indexes[i]]];
			}
		}
		...
		if(n > 0 && streams != nil && (mutableIndexSet = [[NSMutableIndexSet alloc] init]) != nil) {
			for(i = 0; i < n; ++i) {
				[mutableIndexSet addIndex:[[[self document] tcpStreamController] indexForStream:[streams objectAtIndex:i]]];
			}
		}
	...
	if(mutableIndexSet != nil) {
		[streamTableView selectRowIndexes:mutableIndexSet byExtendingSelection:NO];
		[streamTableView scrollRowToVisible:[mutableIndexSet firstIndex]];
		[mutableIndexSet release];
	}

	[streamTableView setIndicatorImage:[[[self document] tcpStreamController] isReverseOrder] ?
...
- (void)tableViewSelectionDidChange:(NSNotification *)aNotification
{
	...
	if(aNotification == nil || [aNotification object] == streamTableView) {
		...
		selectedStream = [[[[self document] tcpStreamController] streamAtIndex:row] retain];
		[packetTableView reloadData];
	}
}
...
{
	if(tableView == packetTableView && [cell isKindOfClass:[NSTextFieldCell class]]) {
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(int)rowIndex
{
	...
	if(tableView == packetTableView) {
		return [self packetTableObjectValueForTableColumn:tableColumn row:rowIndex];
	} else if(tableView == streamTableView) {
		return [self streamTableObjectValueForTableColumn:tableColumn row:rowIndex];
	}
...
{
	if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_IP_ADDRESS])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_SRC_IP_ADDRESS];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_IP_ADDRESS])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_DST_IP_ADDRESS];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_HOSTNAME])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_SRC_HOSTNAME];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_HOSTNAME])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_DST_HOSTNAME];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORT])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_SRC_PORT];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORT])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_DST_PORT];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_SRC_PORTNAME])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_SRC_PORTNAME];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_DST_PORTNAME])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_DST_PORTNAME];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_SENT])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_BYTES_SENT];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_RECV])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_BYTES_RECV];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_BYTES_TOTAL])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_BYTES_TOTAL];
	else if([[lastStreamTableColumn identifier] isEqualToString:PPSTREAMSWINDOW_STREAMS_TABLE_STATUS])
		[[[self document] tcpStreamController] sortStreams:PPSTREAM_SORT_STATUS];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	if([menuItem action] == @selector(deleteButton:) && [[self window] firstResponder] != packetTableView &&
...
- (void)deleteSelectedPackets
{
	...
	if(count == [selectedStream packetsCount]) {
		...
		if([streamTableView selectedRow] == -1) {
			...
			[streamTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:[indexSet firstIndex]] byExtendingSelection:NO];
		...
		[[[self document] tcpStreamController] removePacketsAtIndexes:indexSet forStream:selectedStream];
...
- (void)deleteSelectedStreams
{
	...
	while((n = [indexSet getIndexes:indexes maxCount:(sizeof(indexes) / sizeof(indexes[0])) inIndexRange:&range]) > 0) {
		for(i = 0; i < n; ++i) {
			[[self document] deleteStream:[[[self document] tcpStreamController] streamAtIndex:indexes[i] - adjust] streamIndex:indexes[i] - adjust];
			++adjust;
		}
	}
...
- (IBAction)individualPacketButton:(id)sender
{
	...
	while((n = [indexSet getIndexes:indexes maxCount:(sizeof(indexes) / sizeof(indexes[0])) inIndexRange:&range]) > 0) {
		for(i = 0; i < n; ++i) {
			[[self document] displayIndividualWindow:[selectedStream packetAtIndex:indexes[i]]];
		}
	}
}
...
- (IBAction)reassembleStreamButton:(id)sender
{
	...
	while((n = [indexSet getIndexes:indexes maxCount:(sizeof(indexes) / sizeof(indexes[0])) inIndexRange:&range]) > 0) {
		for(i = 0; i < n; ++i) {
			[[self document] displayReassemblyWindowForPacket:[[[[self document] tcpStreamController] streamAtIndex:indexes[i]] packetAtIndex:0]];
		}
	}
}
...
- (IBAction)autoScrolling:(id)sender
{
	...
	[[NSUserDefaults standardUserDefaults] setBool:autoScrolling forKey:PPSTREAMSWINDOW_AUTOSCROLLING];
}
