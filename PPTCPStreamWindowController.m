...
- (NSString *)string;
- (NSDictionary *)attributesAtIndex:(unsigned int)index effectiveRange:(NSRangePointer)aRange;
...
- (NSDictionary *)attributesAtIndex:(unsigned)attributeIndex effectiveRange:(NSRangePointer)aRange
{
...
- (NSString *)windowTitleForDocumentDisplayName:(NSString *)displayName
{
	...
	[stream hostFrom], [stream srcPort], [stream hostTo], [stream dstPort]];
}
...
- (void)processStreamDataFromChunk:(unsigned int)chunk
{
	...
	redAttributes = [[NSDictionary alloc] initWithObjects:objects forKeys:keys count:ARRAY_NELEMS(keys)];

	objects[0] = [NSColor blueColor];
	blueAttributes = [[NSDictionary alloc] initWithObjects:objects forKeys:keys count:ARRAY_NELEMS(keys)];
	...
	for(; chunk < [reassembler numberOfChunks]; ++chunk) {
		...
		tempString = [[NSString alloc] initWithBytesNoCopy:temp length:[[reassembler chunkDataAt:chunk] length] encoding:NSASCIIStringEncoding freeWhenDone:YES];

		tempAttributedString = [[NSAttributedString alloc] initWithString:tempString attributes:([reassembler chunkIsClient:chunk]) ? redAttributes : blueAttributes];
