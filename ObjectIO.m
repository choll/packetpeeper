...
- (id)initWithFileDescriptor:(int)fdVal
{
    if ((self = [super init]) != nil)
    {
        ...
        {
            [[ErrorStack sharedErrorStack] pushError:@"Could not init"
                                              lookup:[self class]
                                                code:EOBJIO_BADMD
                                            severity:ERRS_ERROR];
            return nil;
        ...
        if ((buf = malloc(READOBJ_BUFSIZ)) == NULL)
        {
            [[ErrorStack sharedErrorStack]
                pushError:@"Failed to allocate memory"
...
- (id<NSCoding>)read
{
    ...
    while (nread < riov[0].iov_len)
    { /* if we dont yet have the length field */
        ...
        if ((nread = readv(fd, riov, 2)) <= 0)
        {
            if (nread == 0) // XXX handle zero properly in the error handler
                errno = 0;
            [[ErrorStack sharedErrorStack]
                pushError:@"Failed to readv ObjectIO data"
                   lookup:[PosixError class]
                     code:errno
                 severity:ERRS_ERROR];
            return nil;
    ...
    nread -=
        riov[0]
            .iov_len; /* update nread so it does not include anything of 'len' */
    ...
    if (rlen > LEN_MAX)
    {
        ...
        [[ErrorStack sharedErrorStack]
            pushError:@"Could not handle input ObjectIO data"
               lookup:[self class]
                 code:EOBJIO_BADLEN
             severity:ERRS_ERROR];
        return nil;
    ...
    { /* if we did not yet read the full object, read the rest in */
        if (rlen > buflen)
        { /* allocate more memory if needed */
            ...
            if ((temp = realloc(buf, rlen)) == NULL)
            {
                free(buf);
                [[ErrorStack sharedErrorStack]
                    pushError:@"Failed to realloc extra memory"
                       lookup:[PosixError class]
                         code:errno
                     severity:ERRS_ERROR];
                return nil;
        ...
        while (nleft > 0)
        {
            ...
            {
                [[ErrorStack sharedErrorStack]
                    pushError:@"Failed to read ObjectIO data"
                       lookup:[PosixError class]
                         code:errno
                     severity:ERRS_ERROR];
                return nil;
    ...
    if (nleft)
    {                  /* if we have extra data to deal with */
        ...
        if (nread > sizeof(size_t))
        {
            ...
            (void)memmove(
                buf,
                (uint8_t*)buf + prevlen + sizeof(size_t),
                nread - sizeof(size_t));
        }
        else
...
- (ssize_t)write:(id<NSCoding>)obj
{
    ...
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"Could not handle output ObjectIO data"
               lookup:[self class]
                 code:EOBJIO_BADLEN
             severity:ERRS_ERROR];
        return -1;
    ...
    if ((ret = writevn(fd, wiov, 2)) <= 0)
    {
        if (ret == 0)
            errno = 0;
        [[ErrorStack sharedErrorStack]
            pushError:@"Failed to writevn ObjectIO data"
               lookup:[PosixError class]
                 code:errno
             severity:ERRS_ERROR];
        return ret;
