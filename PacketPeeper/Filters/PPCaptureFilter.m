...
- (PPBPFProgram*)filterProgramForLinkType:(int)linkType
{
    ...
    if (pcap_compile(
            cap, &bpf_program, (char*)[m_filter UTF8String], 1, m_netmask) ==
        -1)
    {
        const char* err;
    ...
    pcap_freecode(
        &bpf_program); /* pcap_close() probably does this for us, but the docs dont say so */
    pcap_close(cap);
...
- (id)copyWithZone:(NSZone*)zone
{
    ...
    copy = [[[self class] allocWithZone:zone] initWithTCPDumpFilter:filterCopy
                                                               name:nameCopy];
