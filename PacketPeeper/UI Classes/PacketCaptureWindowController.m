...
#define PACKETCAPTURE_TOOLBAR_ID \
    @"PacketCaptureToolbar" /* identifier for the NSToolbar itself */
...
#define CLEAR_FILTER_TOOLBARITEM_ID \
    @"ClearFilterItem" /* ``clear filters'' button */
...
- (void)selectPacketAtIndex:(NSUInteger)index
{
    if ([packetTableView numberOfRows] > index)
        [packetTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:index]
...
- (void)windowDidLoad
{
    autoScrolling = [[NSUserDefaults standardUserDefaults]
        boolForKey:PPDOCUMENT_AUTOSCROLLING];
    lastColumn = nil;
    ...
    [[packetTableView headerView]
        setMenu:[(AppController*)[NSApp delegate] createProtocolsMenu]];
...
- (void)doubleAction:(id)sender
{
    ...
    if ((row = [sender clickedRow]) != -1)
        [[self document]
            displayIndividualWindow:[[self document] packetAtIndex:row]];
}
...
- (void)populatePacketTableView
{
    ...
    if ((arrayAsData = [[NSUserDefaults standardUserDefaults]
             objectForKey:PPDOCUMENT_TABLEVIEW_COLUMNS_KEY]) == nil)
        return;
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wincompatible-pointer-types"
    return [[[self document] packetAtIndex:rowIndex]
...
{
    [self setPacket:[[self document]
                        packetAtIndex:[packetTableView selectedRow]]];
}
    ...
    didClickTableColumn:(NSTableColumn*)tableColumn
{
    ...
    if (lastColumn == tableColumn)
    {
        ...
        [[self document]
            setReversePacketOrder:![[self document] isReverseOrder]];
    }
    else
    {
        ...
        if ((packets = [[NSMutableArray alloc] init]) != nil)
        {
                             ...
                             inIndexRange:&range]) > 0)
            {
                for (i = 0; i < n; ++i)
                    [packets
                        addObject:[[self document] packetAtIndex:indexes[i]]];
            }
        }
            ...
            (mutableIndexSet = [[NSMutableIndexSet alloc] init]) != nil)
        {
            ...
            {
                [mutableIndexSet
                    addIndex:[[self document]
                                 indexForPacket:[packets objectAtIndex:i]]];
            }
        }
    ...
    {
        [packetTableView selectRowIndexes:mutableIndexSet
                     byExtendingSelection:NO];
    ...
    [packetTableView
        setIndicatorImage:[[self document] isReverseOrder]
                              ? [NSImage
...
- (NSArray*)toolbarAllowedItemIdentifiers:(NSToolbar*)toolbar
{
    return [NSArray arrayWithObjects:NSToolbarSeparatorItemIdentifier,
                                     ...
                                     NEXT_TOOLBARITEM_ID,
                                     PREV_TOOLBARITEM_ID,
                                     FIRST_TOOLBARITEM_ID,
                                     LAST_TOOLBARITEM_ID,
                                     DELETE_TOOLBARITEM_ID,
                                     STOP_TOOLBARITEM_ID,
                                     FILTER_TOOLBARITEM_ID,
                                     CLEAR_FILTER_TOOLBARITEM_ID,
                                     nil];
}

- (NSArray*)toolbarDefaultItemIdentifiers:(NSToolbar*)toolbar
{
    return [NSArray arrayWithObjects:FIRST_TOOLBARITEM_ID,
                                     PREV_TOOLBARITEM_ID,
                                     NEXT_TOOLBARITEM_ID,
                                     LAST_TOOLBARITEM_ID,
...
- (BOOL)validateToolbarItem:(NSToolbarItem*)theItem
{
    ...
    if ([[theItem itemIdentifier] isEqualToString:NEXT_TOOLBARITEM_ID])
    {
        NSInteger row;
        return (
            (row = [packetTableView selectedRow]) != -1 &&
            row != ([packetTableView numberOfRows] - 1));
    }

    if ([[theItem itemIdentifier] isEqualToString:FIRST_TOOLBARITEM_ID])
        return (
            [packetTableView selectedRow] != 0 &&
            [packetTableView numberOfRows] != 0);
...
{
    if ([menuItem action] == @selector(arpSpoofingWindowButton:) &&
    ...
    if ([menuItem action] == @selector(stopButton:) && !
                                                       [[self document] isLive])
        return NO;
    ...
    if ([menuItem action] == @selector(reassembleStreamButton:))
    {
        ...
        if ((segment =
                 [[[self document] packetAtIndex:[packetTableView selectedRow]]
...
- (void)update:(BOOL)shouldScroll
{
    if ([lastColumn identifier] != [Packet class] &&
        [(ColumnIdentifier*)[lastColumn identifier] index] !=
            PACKET_COLUMN_INDEX_NUMBER &&
        [(ColumnIdentifier*)[lastColumn identifier] index] !=
            PACKET_COLUMN_INDEX_DATE)
    {
        [self sortPackets];
    }
    ...
    [statusTextField
        setStringValue:[NSString
                           stringWithFormat:@"%zu packets, %@",
                                            [[self document] numberOfPackets],
                                            data_quantity_str([[self document]
                                                numberOfBytes])]];
...
- (IBAction)autoScrolling:(id)sender
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:autoScrolling
...
- (IBAction)columnMenuAction:(id)sender
{
    ...
    else if ([packetTableView numberOfColumns] > 1)
    { /* the user must have at least one column */
        [packetTableView
            removeTableColumn:[packetTableView tableColumnWithIdentifier:
...
- (IBAction)individualPacketButton:(id)sender
{
                        ...
                        inIndexRange:&range]) > 0)
    {
        ...
        {
            [[self document]
                displayIndividualWindow:[[self document]
