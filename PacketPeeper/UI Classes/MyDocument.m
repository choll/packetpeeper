...
- (void)makeWindowControllers
{
    ...
    {
        captureWindowController = [[PacketCaptureWindowController alloc]
             ...
             error:(NSError**)outError
{
    ...
    NSMutableArray* savePackets = [NSMutableArray
        arrayWithArray:allPackets != nil ? allPackets
                                         : packets]; // autoreleased
    ...
    const BOOL stripPkTap =
        ...
        [[[[savePackets objectAtIndex:0] decoders] objectAtIndex:0] class] ==
            [PPRVIDecode class] &&
        !
        [interface isEqualToString:
                       @"pcap"]; // interface is pcap if capture was loaded from disk
    ...
    {
        saveLinkType =
            [[[[savePackets objectAtIndex:0] decoders] objectAtIndex:0] dlt];
    }
    else
    {
    ...
    for (Packet* pkt in savePackets)
    {
        ...
        hdr.ts.tv_usec = (suseconds_t)(
            modf([[pkt date] timeIntervalSince1970], &seconds) * 1000000.0);
        ...
        if (stripPkTap &&
            [[[pkt decoders] objectAtIndex:0] class] == [PPRVIDecode class])
        {
            ...
            pcap_dump(
                (unsigned char*)dump,
                &hdr,
                (unsigned char*)[[pkt packetData] bytes] + offset);
        }
        else
        {
            pcap_dump(
                (unsigned char*)dump,
                &hdr,
                (unsigned char*)[[pkt packetData] bytes]);
        }
    }
...
err:
    ...
    *outError = [[NSError alloc] initWithDomain:@"PacketPeeperErrorDomain"
                                           code:noErr
                                       userInfo:errDict];
              ...
              error:(NSError**)outError
{
    ...
    if ((thread_args = malloc(sizeof(struct thread_args))) == NULL)
    {
        errorString = [NSString
            stringWithFormat:@"Error: malloc failed: %s", strerror(errno)];
        goto err;
    }
    ...
    if ((ret = pthread_create(
             ...
             NULL,
             read_from_url_thread,
             thread_args)) != 0)
    {
        errorString =
            [NSString stringWithFormat:@"Error: failed to create thread: %s",
                                       strerror(ret)];
    ...
    progressWindowController = [[PPProgressWindowController alloc]
        initWithLoadingMessage:@"Loading"
                      delegate:self
                cancelSelector:@selector(cancelLoadingFile)];
    [self performSelector:@selector(displayFileLoadingProgressSheet)
               withObject:self
               afterDelay:0];

    thread_args->timer = [[NSTimer
        scheduledTimerWithTimeInterval:DEFAULT_PROGRESSBAR_UPDATE_FREQUENCY
                                target:self
                              selector:@selector(workerThreadTimer)
                              userInfo:nil
                               repeats:YES] retain];
    return YES;
...
- (void)workerThreadTimer
{
    ...
    {
        const double percentLoaded =
            thread_args->units_current / (double)thread_args->units_total;
    ...
    if (thread_args->success || thread_args->failure)
    {
        ...
        if ((ret = pthread_join(thread_args->thread_id, NULL)) != 0)
        {
            ...
            errorString =
                [NSString stringWithFormat:@"pthread_join(%p) failed",
                                           (void*)thread_args->thread_id];
            [[ErrorStack sharedErrorStack] pushError:errorString
        ...
        if (thread_args->success)
        {
            ...
            if (thread_args->op == THREAD_OP_DOC_READ)
            {
                ...
                [packets makeObjectsPerformSelector:@selector(setDocument:)
                                         withObject:self];
        ...
        else if (thread_args->failure)
        {
            if (thread_args->op == THREAD_OP_DOC_READ)
            {
                ...
                [[ErrorStack sharedErrorStack]
                    pushError:[NSString stringWithFormat:
                                            @"Loading capture file failed: %@",
                                            thread_args->output[0]]
                       lookup:Nil
                         code:0
                     severity:ERRS_ERROR];
...
- (void)cancelWorkerThread
{
    ...
    if ((ret = pthread_join(thread_args->thread_id, &output)) != 0)
    {
        ...
        errorString = [NSString stringWithFormat:@"pthread_join(%p) failed",
                                                 (void*)thread_args->thread_id];
        [[ErrorStack sharedErrorStack] pushError:errorString
                                          lookup:[PosixError class]
                                            code:ret
...
- (void)closeProgressSheet
{
    ...
    {
        [[self windowForSheet] endSheet:[progressWindowController window]
...
- (void)displayFileLoadingProgressSheet
{
    ...
    [self displayProgressSheetWithMessage:@"Loading"
                           cancelSelector:@selector(cancelLoadingFile)];
}
                         ...
                         cancelSelector:(SEL)cancelSelector
{
    ...
    if (progressWindowController == nil)
        progressWindowController = [[PPProgressWindowController alloc]
            initWithLoadingMessage:message
                          delegate:self
                    cancelSelector:cancelSelector];

    [self addWindowController:progressWindowController];

    [[self windowForSheet] beginSheet:[progressWindowController window]
...
- (void)displayFilterSheet
{
    ...
    [[self windowForSheet] beginSheet:[filterWindowController window]
                    completionHandler:^(NSModalResponse result) {
                      [filterWindowController
                          sheetDidEnd:[filterWindowController window]
                           returnCode:result
                          contextInfo:NULL];
                    }];
...
- (void)displaySetupSheet
{
    ...
    [[self windowForSheet] beginSheet:[setupWindowController window]
                    completionHandler:^(NSModalResponse result) {
                      [setupWindowController
                          sheetDidEnd:[setupWindowController window]
                           returnCode:result
                          contextInfo:NULL];
                    }];
...
- (void)displayIndividualWindow:(Packet*)aPacket
{
    ...
    for (i = 0; i < [controllersArray count]; ++i)
    {
        ...
        if ([controller
                isMemberOfClass:[IndividualPacketWindowController class]] &&
            [controller packet] == aPacket)
        {
    ...
    controller =
        [[IndividualPacketWindowController alloc] initWithPacket:aPacket];
...
- (void)displayReassemblyWindowForPacket:(Packet*)aPacket
{
    ...
    if ((reassembler = [[self tcpStreamController]
             streamReassemblerForPacket:aPacket]) == nil)
        return;

    for (i = 0; i < [controllersArray count]; ++i)
    {
        controller = [controllersArray objectAtIndex:i];
        if ([controller isMemberOfClass:[PPTCPStreamWindowController class]] &&
            [controller streamReassembler] == reassembler)
        {
            [controller showWindow:self];
            return;
        }
    ...
    controller =
        [[PPTCPStreamWindowController alloc] initWithReassembler:reassembler];
...
- (void)displayArpSpoofingWindow
{
    ...
    {
        arpSpoofingWindowController =
            [[PPArpSpoofingWindowController alloc] init];
...
- (void)addPacketArray:(NSArray*)packetArray
{
    ...
    if (progressWindowController != nil)
        [progressWindowController
            setLoadingMessage:[NSString
                                  stringWithFormat:@"Loading, %lu packets",
                                                   packetCount]];
}
...
{
    NSUInteger upper, lower, current, total;
    ...
    for (;;)
    {
        ...
        switch (
            pkt_compare([packets objectAtIndex:current], packet, sortColumn))
        {
        /* current is greater */
        ...
        default:
            ...
            for (upper = current; [packets objectAtIndex:upper] != packet;
                 ++upper)
            {
                if (upper == total - 1)
                    return -1;
                ...
                if (pkt_compare(
                        [packets objectAtIndex:upper + 1],
                        packet,
                        sortColumn) != NSOrderedSame)
                {
                    for (upper = current - 1;
                         [packets objectAtIndex:upper] != packet;
                         --upper)
                    {
                        if (upper == 0)
                            return -1;
...
- (void)displayErrorStack:(ErrorStack*)errorStack close:(BOOL)closeDocument
{
    ...
    [[self windowForSheet]
        ...
        completionHandler:^(NSModalResponse result) {
          [self
              sheetDidEnd:[errorReportWindowController window]
               returnCode:result
              contextInfo:
                  (void*)((intptr_t)
                              closeDocument) /* disgust, intptr is to suppress warning... */
          ];
        }];
}
...
{
    if ([[sheet windowController]
            isMemberOfClass:[ErrorReportWindowController class]])
    {
                ...
                filter:(PPCaptureFilter*)filter
{
    ...
    if (stopDate != nil)
        endingTimer = [NSTimer
            scheduledTimerWithTimeInterval:[stopDate timeIntervalSinceNow]
                                    target:self
                                  selector:@selector(endCaptureWithTimer:)
                                  userInfo:nil
                                   repeats:NO];
    ...
    if ((sockfd = [[MyDocumentController sharedDocumentController]
             launchHelper]) == -1)
        goto err;
    ...
    if ((sockref = CFSocketCreateWithNative(
             kCFAllocatorDefault,
             sockfd,
    ...
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"Call to CFSocketCreateWithNative failed"
               lookup:Nil
                 code:0
             severity:ERRS_ERROR];
        goto err;
    ...
    if ((source = CFSocketCreateRunLoopSource(
             kCFAllocatorDefault, sockref, 0)) == NULL)
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"Call to CFSocketCreateRunLoopSource failed"
               lookup:Nil
                 code:0
             severity:ERRS_ERROR];
        goto err;
...
- (void)readData
{
    ...
    do
    {
        ...
        if ([obj isMemberOfClass:[Packet class]])
        {
            ...
            if (endingBytes > 0)
            {
                if (endingBytes <= [obj actualLength])
                {
                    endingBytes = 0;
                    if (!endingMatchAll ||
                        (endingPackets == 0 && endingTimer != nil))
                        [self stopCapture];
                }
                else
        ...
        else
        {
            ...
            {
                [[ErrorStack sharedErrorStack]
                    pushError:@"Received unknown object from helper tool"
                       lookup:Nil
                         code:0
                     severity:ERRS_ERROR];
                obj = nil;
    ...
    if (timer == nil || ![timer isValid])
    {
        [timer release];
        timer = [[NSTimer
            scheduledTimerWithTimeInterval:DEFAULT_UI_UPDATE_FREQUENCY
                                    target:self
                                  selector:@selector(updateControllerWithTimer:)
                                  userInfo:nil
                                   repeats:NO] retain];
    }
...
err:
    [self stopCapture];
    [self displayErrorStack:obj
                      close:([packets count] == 0)
                                ? YES
                                : NO]; /* close if no packets received */
}
...
- (void)setCaptureFilter:(PPCaptureFilter*)captureFilter
{
    ...
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"File loading or saving operation already in progress"
               lookup:Nil
                 code:errno
             severity:ERRS_ERROR];
        goto err;
    ...
    if ((thread_args = malloc(sizeof(struct thread_args))) == NULL)
    {
        [[ErrorStack sharedErrorStack] pushError:@"Failed to allocate memory"
                                          lookup:[PosixError class]
                                            code:errno
                                        severity:ERRS_ERROR];
        goto err;
    ...
    [bpfProgram release];
    bpfProgram =
        [[captureFilter filterProgramForLinkType:[self linkType]] retain];
    ...
    [self displayProgressSheetWithMessage:@"Filtering"
                           cancelSelector:@selector
                           (cancelCaptureFilterExecution)];

    thread_args->op = THREAD_OP_DOC_FILTER;
    thread_args->input[0] = [[NSArray alloc] initWithArray:allPackets];
    thread_args->input[1] = [bpfProgram
        retain]; /* this is kind of lame, as we've already retained above, but
    ...
    if ((ret = pthread_create(
             &thread_args->thread_id,
             NULL,
             filter_packets_thread,
             thread_args)) != 0)
    {
        [[ErrorStack sharedErrorStack] pushError:@"Failed to create thread"
                                          lookup:[PosixError class]
                                            code:ret
                                        severity:ERRS_ERROR];
        [thread_args->input[0] release];
...
static void* read_from_url_thread(void* args)
{
    ...
    if ((pcap = pcap_open_offline(
             [[(NSURL*)thread_args->input[0] path] UTF8String], errbuf)) ==
        NULL)
    {
    ...
    if ((fp = pcap_file(pcap)) != NULL && (fd = fileno(fp)) != -1 &&
        fstat(fd, &sb) == 0)
    ...
    for (packet_number = 1; (ret = pcap_next_ex(pcap, &hdr, &bytes)) == 1;
         ++packet_number)
    {
        NSData* data;
        Packet* packet;
        ...
        packet = [[Packet alloc] initWithData:data
                                captureLength:hdr->caplen
                                 actualLength:hdr->len
                                    timestamp:TIMEVAL_TO_NSDATE(hdr->ts)
                                    linkLayer:linkType];
...
static void* filter_packets_thread(void* args)
{
    ...
    for (thread_args->units_current = 0;
         thread_args->units_current < thread_args->units_total;
         ++thread_args->units_current)
    {
        Packet* packet;

        packet = [(NSArray*)thread_args->input[0]
            objectAtIndex:thread_args->units_current];
    ...
    streamController = [[PPTCPStreamController alloc] init];
    tempPackets = [filteredPackets sortedArrayUsingFunction:pkt_compare
                                                    context:nil];
