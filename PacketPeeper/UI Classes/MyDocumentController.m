...
#else
#define HELPER_PATH                                                           \
    [[[NSBundle mainBundle] pathForAuxiliaryExecutable:@"PacketPeeperHelper"] \
        UTF8String]
#endif
...
- (IBAction)terminate:(id)sender
{
    ...
    {
        if ([[[windows objectAtIndex:i] windowController]
                isMemberOfClass:[CaptureSetupWindowController class]] ||
            [[[windows objectAtIndex:i] windowController]
                isMemberOfClass:[PPCaptureFilterWindowController class]] ||
            [[[windows objectAtIndex:i] windowController]
                isMemberOfClass:[PPProgressWindowController class]])
        {
            [[[windows objectAtIndex:i] windowController]
...
- (IBAction)newDocument:(id)sender
{
    AuthorizationItem items = {
        kAuthorizationRightExecute, 0, NULL, 0}; /* right to exec as root */
...
- (BOOL)createAuthRef
{
    ...
    if (!auth_ref)
    {
        if (AuthorizationCreate(
                ...
                &auth_ref) != errAuthorizationSuccess)
        {
            [[ErrorStack sharedErrorStack]
                pushError:@"Failed to obtain authorization reference"
                   lookup:Nil
                     code:0
                 severity:ERRS_ERROR];
            return NO;
...
- (BOOL)listenSetup
{
    if (listenfd == -1)
    {
        ...
        if (bind(listenfd, (struct sockaddr*)&servaddr, sizeof(servaddr)) == -1)
        {
            (void)close(listenfd);
            listenfd = -1;
            [[ErrorStack sharedErrorStack] pushError:@"Could not bind socket"
        ...
        { // XXX config.h?
            ...
            listenfd = -1;
            [[ErrorStack sharedErrorStack]
                pushError:@"Could not listen on socket"
                   lookup:[PosixError class]
                     code:errno
                 severity:ERRS_ERROR];
            return NO;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    if (AuthorizationExecuteWithPrivileges(
            auth_ref, HELPER_PATH, kAuthorizationFlagDefaults, &args, NULL) !=
        errAuthorizationSuccess)
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"Could not execute helper tool"
               lookup:Nil
                 code:0
             severity:ERRS_ERROR];
        return -1;
...
#pragma clang diagnostic pop
    ...
    case -1:
        [[ErrorStack sharedErrorStack]
            pushError:@"Error when waiting for helper tool to connect"
               lookup:[PosixError class]
                 code:errno
             severity:ERRS_ERROR];
        return -1;
    ...
    case 0:
        [[ErrorStack sharedErrorStack]
            pushError:@"Timed out waiting for helper tool to connect"
               lookup:Nil
                 code:0
             severity:ERRS_ERROR];
        return -1;
    ...
    {
        [[ErrorStack sharedErrorStack]
            pushError:@"Could not accept connection on socket"
               lookup:[PosixError class]
                 code:errno
             severity:ERRS_ERROR];
        return -1;
