...
- (NSString *)info
{
	return [NSString stringWithFormat:@"%u to %u, [%@], S=%u, A=%u, %uB payload%s",
									  sport,
									  dport,
									  [self flagsStr], seq_no, ack_no, [self size], [self isChecksumValid] ? "" : " (bad checksum)"];
}
...
- (NSString *)flagsStr
{
	NSString *flag_names[] = {@"FIN", @"SYN", @"RST", @"PSH", @"ACK", @"URG", @"ECE", @"CWR"};
	NSMutableString *ret;
	unsigned int i;
...
- (uint16_t)computedChecksum
{
	...
	if([data length] < skip_bytes || [data length] - skip_bytes < TCPDECODE_HDR_MIN)
		return 0;
	...
	pseudo_hdr.len = htons(pseudo_hdr_nbytes);
...
- (uint32_t)size
{
	...
	for(i = 0; i < [decoders count]; ++i) {
		...
		if(iplen == 0 && [[decoders objectAtIndex:i] isMemberOfClass:[IPV4Decode class]]) {
			iplen = [[decoders objectAtIndex:i] length];

			/* check for bogus IP length */
			if(iplen <= [[decoders objectAtIndex:i] frontSize] || iplen > [parent captureLength] - front)
				return 0;
	...
	for(i = 0; i < [decoders count]; ++i) {
		...
		if([decoders objectAtIndex:i] == self) {
			if(iplen == front) /* no payload present */
				return nil;
			return [NSData dataWithBytesNoCopy:((uint8_t *)[[parent packetData] bytes] + htotal) length:iplen - front freeWhenDone:NO];
		}
	}
...
{
	return (self == segment || (sport == [segment srcPort] &&
	   dport == [segment dstPort] &&
	   seq_no == [segment seqNo] &&
	   ack_no == [segment ackNo] &&
	   flags == [segment flags] &&
	   [self size] == [segment size])) ? YES : NO;
}

- (NSString *)description
{
	return [NSString stringWithFormat:@"[TCP %@: S: %u, A: %u, %uB]", [self flagsStr], seq_no, ack_no, [self size]];
}
...
- (id)childAtIndex:(int)fieldIndex
{
	...
	switch(fieldIndex) {
		case 0:
			...
			[ret addObject:str];
			[str release];
			if((str = [[PortCache sharedPortCache] serviceWithTCPPort:sport]) == nil)
				str = @"Lookup failed";
		...
		case 1:
			...
			[ret addObject:str];
			[str release];
			if((str = [[PortCache sharedPortCache] serviceWithTCPPort:dport]) == nil)
				str = @"Lookup failed";
		...
		case 5:
			str = [[NSString alloc] initWithFormat:@"0x%.2x (%@)", flags, [self flagsStr]];
			[ret addObject:str];
			[str release];
			[ret addChildWithObjects:[NSString stringWithFormat:@"ECN-Echo (ECE): %s", (flags & TH_ECE) ? "set" : "unset"],
									 [NSString stringWithFormat:@"%u... ....", (flags & TH_ECE) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Congestion window reduction (CWR): %s", (flags & TH_CWR) ? "set" : "unset"],
									 [NSString stringWithFormat:@".%u.. ....", (flags & TH_CWR) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Urgent pointer (URG): %s", (flags & TH_URG) ? "set" : "unset"],
									 [NSString stringWithFormat:@"..%u. ....", (flags & TH_URG) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Acknowledgement number (ACK): %s", (flags & TH_ACK) ? "set" : "unset"],
									 [NSString stringWithFormat:@"...%u ....", (flags & TH_ACK) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Push (PSH): %s", (flags & TH_PUSH) ? "set" : "unset"],
									 [NSString stringWithFormat:@".... %u...", (flags & TH_PUSH) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Reset connection (RST): %s", (flags & TH_RST) ? "set" : "unset"],
									 [NSString stringWithFormat:@".... .%u..", (flags & TH_RST) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"Synchronize sequence numbers (SYN): %s", (flags & TH_SYN) ? "set" : "unset"],
									 [NSString stringWithFormat:@".... ..%u.", (flags & TH_SYN) != 0], nil];
			[ret addChildWithObjects:[NSString stringWithFormat:@"End of data (FIN): %s", (flags & TH_FIN) ? "set" : "unset"],
									 [NSString stringWithFormat:@".... ...%u", (flags & TH_FIN) != 0], nil];
			break;
			/* NOTREACHED */
