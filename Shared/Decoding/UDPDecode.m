...
- (NSString*)info
{
    return [NSString stringWithFormat:@"%u to %u, %u B", sport, dport, len];
}
...
- (uint16_t)computedChecksum
{
    ...
    if ([data length] < skip_bytes ||
        [data length] - skip_bytes < UDPDECODE_HDR_MIN)
        return 0;
    ...
    partial_sum = in_cksum_partial(
        hdr,
        self->len,
        partial_sum); // XXX what if packet is truncated, need to cap to real sz
...
- (NSComparisonResult)compareWith:(id)obj atIndex:(unsigned int)fieldIndex
{
    ...
    case 4:
        return [[[PortCache sharedPortCache] serviceWithUDPPort:sport]
            compare:[[PortCache sharedPortCache]
                        serviceWithUDPPort:((UDPDecode*)obj)->sport]];
    case 5:
        return [[[PortCache sharedPortCache] serviceWithUDPPort:dport]
            compare:[[PortCache sharedPortCache]
                        serviceWithUDPPort:((UDPDecode*)obj)->dport]];
    }
...
- (id)childAtIndex:(int)fieldIndex
{
    ...
    case 0:
        ...
        [ret addObject:str];
        [str release];
        if ((str = [[PortCache sharedPortCache] serviceWithUDPPort:sport]) ==
            nil)
            str = @"Lookup failed";
    ...
    case 1:
        ...
        [ret addObject:str];
        [str release];
        if ((str = [[PortCache sharedPortCache] serviceWithUDPPort:dport]) ==
            nil)
            str = @"Lookup failed";
