...
static NSString* arp_names[][2] = {
    {@"Hardware Address Type", @"ARP HW Type"},
...
static NSString* rarp_names[][2] = {
    {@"Hardware Address Type", @"RARP HW Type"},
...
- (id)initWithData:(NSData*)dataVal parent:(id<PPDecoderParent>)parentVal
{
    ...
    if ((self = [super init]) != nil)
    {
        ...
        if (hardtype == ARPHRD_ETHER && hardsz == ETHER_ADDR_LEN)
        {
            (void)memcpy(
                ethsender,
                (void*)((uintptr_t)hdr + ARP_FIELD_SZ),
                sizeof(ethsender)); // pull back by 4 bytes?
            (void)memcpy(
                ethtarget,
                (void*)((uintptr_t)hdr + ARP_FIELD_SZ + ETHER_ADDR_LEN + protsz),
                sizeof(ethtarget));
        }
        ...
        if (prottype == ETHERTYPE_IP && protsz == sizeof(struct in_addr))
        {
            (void)memcpy(
                &ipsender,
                (void*)((uintptr_t)hdr + ARP_FIELD_SZ + hardsz),
                sizeof(ipsender));
            (void)memcpy(
                &iptarget,
                (void*)((uintptr_t)hdr + ARP_FIELD_SZ + (hardsz * 2) + sizeof(struct in_addr)),
                sizeof(iptarget));
        }
    }
    return self;
...
- (NSString*)info
{
    ...
    case ARPOP_REPLY:
        return [NSString
            stringWithFormat:@"%@ is at %@", [self sender], [self senderEther]];

    case ARPOP_REVREQUEST:
        return [NSString stringWithFormat:@"Who is %@? Tell %@",
                                          [self targetEther],
                                          [self senderEther]];

    case ARPOP_REVREPLY:
        return [NSString
            stringWithFormat:@"%@ is at %@", [self targetEther], [self target]];
    }
...
{
    if (prottype == ETHERTYPE_IP && protsz == sizeof(struct in_addr))
        return [[parent hostCache] hostWithAddressASync:&ipsender
                                             returnCode:NULL];
    else
        return nil;
}
...
{
    if (prottype == ETHERTYPE_IP && protsz == sizeof(struct in_addr))
        return [[parent hostCache] hostWithAddressASync:&iptarget
                                             returnCode:NULL];
    else
        return nil;
}
...
- (NSString*)columnStringForIndex:(unsigned int)fieldIndex
{
    ...
    case 15:
        if (hardtype == ARPHRD_ETHER && hardsz == ETHER_ADDR_LEN)
            return [[OUICache sharedOUICache]
                manufacturerForEthernetAddress:ethsender];
        else
    ...
    case 16:
        if (hardtype == ARPHRD_ETHER && hardsz == ETHER_ADDR_LEN)
            return [[OUICache sharedOUICache]
                manufacturerForEthernetAddress:ethtarget];
        else
            return @"Unknown hardware";
        /* NOTREACHED */
    }
...
- (NSComparisonResult)compareWith:(id)obj atIndex:(unsigned int)fieldIndex
{
    ...
    case 5:
        return mem_compare(
            ethsender, ((ARPDecode*)obj)->ethsender, sizeof(ethsender));
        /* NOTREACHED */

    case 6:
        return mem_compare(
            &ipsender, (&((ARPDecode*)obj)->ipsender), sizeof(ipsender));
        /* NOTREACHED */

    case 7:
        return mem_compare(
            ethtarget, ((ARPDecode*)obj)->ethtarget, sizeof(ethtarget));
        /* NOTREACHED */

    case 8:
        return mem_compare(
            &iptarget, (&((ARPDecode*)obj)->iptarget), sizeof(iptarget));
        /* NOTREACHED */
        ...
        if (hardtype == ARPHRD_ETHER && hardsz == ETHER_ADDR_LEN)
            return [[[OUICache sharedOUICache]
                ...
                compare:[[OUICache sharedOUICache]
                            manufacturerForEthernetAddress:((ARPDecode*)obj)
                                                               ->ethsender]];
        break;
...
- (id)childAtIndex:(int)fieldIndex
{
    ...
    case 4:
        ...
        [ret addChildWithCallback:self
                         selector:@selector(operationStringCallback:)
                             data:NULL];
        break;
    ...
    case 5:
        ...
        if ((str = [[OUICache sharedOUICache]
                 manufacturerForEthernetAddress:ethsender]) == nil)
            str = @"Lookup failed";
    ...
    case 7:
        ...
        if ((str = [[OUICache sharedOUICache]
                 manufacturerForEthernetAddress:ethtarget]) == nil)
            str = @"Lookup failed";
...
+ (NSArray*)columnIdentifiers
{
    ...
    ret = [[NSMutableArray alloc]
        initWithCapacity:sizeof(rarp_names) / sizeof(rarp_names[0])];

    for (i = 0; i < sizeof(rarp_names) / sizeof(rarp_names[0]); ++i)
    {
        colIdent = [[ColumnIdentifier alloc] initWithDecoder:[self class]
                                                       index:i
                                                    longName:rarp_names[i][0]
                                                   shortName:rarp_names[i][1]];
        [ret addObject:colIdent];
        [colIdent release];
    }
