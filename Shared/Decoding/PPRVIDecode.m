...
static NSString* names[][2] = {{@"Length", @"RVI Len"},
...
{
    if (data == nil || [data length] < sizeof(struct pp_pktap_header) ||
        (self = [super init]) == nil)
    {
        return nil;
    }
...
- (void)setParent:(id<PPDecoderParent>)parent
{
    ...
    m_hdr =
        (struct
         pp_pktap_header*)((char*)[[m_parent packetData] bytes] + [m_parent byteOffsetForDecoder:self]);
}
...
- (NSComparisonResult)compareWith:(id)obj atIndex:(unsigned int)fieldIndex
{
    ...
    case 5:
        return val_compare(
            m_hdr->pth_protocol_family, other->pth_protocol_family);
    case 6:
        return val_compare(
            m_hdr->pth_frame_pre_length, other->pth_frame_pre_length);
    case 7:
        return val_compare(
            m_hdr->pth_frame_post_length, other->pth_frame_post_length);
    case 8:
...
- (id)childAtIndex:(int)fieldIndex
{
    ...
    case 5:
        str =
            [[NSString alloc] initWithFormat:@"%u", m_hdr->pth_protocol_family];
        [ret addObject:str];
        [str release];
        break;
    case 6:
        str = [[NSString alloc]
            initWithFormat:@"%u", m_hdr->pth_frame_pre_length];
        [ret addObject:str];
        [str release];
        break;
    case 7:
        str = [[NSString alloc]
            initWithFormat:@"%u", m_hdr->pth_frame_post_length];
        [ret addObject:str];
        [str release];
        break;
    case 8:
