...
{
    return [NSString
        stringWithFormat:@"%@ (%@)", [self codeString], [self infoType]];
}
...
- (NSString*)codeString
{
    ...
    case ICMP_UNREACH: /* dest unreachable, codes: */
        ...
        case ICMP_UNREACH_NEEDFRAG: /* IP_DF caused drop */
            return @"Destination unreachable: Fragmentation needed but "
        ...
        case ICMP_UNREACH_NET_PROHIB: /* prohibited access */
            return @"Destination unreachable: Destination network "
                   @"administratively prohibited";
        case ICMP_UNREACH_HOST_PROHIB: /* prohibited access */
            return @"Destination unreachable: Destination host "
                   @"administratively prohibited";
        ...
        case ICMP_UNREACH_FILTER_PROHIB: /* admin prohib */
            return @"Destination unreachable: Communication administratively "
...
{
    if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_GWADDR)
        return [[parent hostCache] hostWithAddressASync:&cont.upper.gateway
                                             returnCode:NULL];
    ...
    case 7:
        if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_SUBMASK)
            return ipaddrstr(
                ...
                sizeof(
                    cont.lower
                        .mask)); // [NSString stringWithFormat:@"%u", cont.lower.mask];
        break;
...
{
    return 3 +
           (((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ) *
            2) + /* only one of these 3 is present */
           ...
           (((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_UDP) *
            2) + /* UDP and TCP include IP */
...
- (id)childAtIndex:(int)fieldIndex
{
    ...
    case 3:
        if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ)
        {
            ...
            str =
                [[NSString alloc] initWithFormat:@"%u", cont.upper.idseq.ident];
            [ret addObject:str];
            [str release];
        }
    ...
    case 4:
        ...
        else if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TSTAMP)
        {
            ...
            str =
                [[NSString alloc] initWithFormat:@"%u", cont.lower.tstamp.orig];
            [ret addObject:str];
            [str release];
        }
        else if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_SUBMASK)
        {
            ...
            [ret
                addObject:ipaddrstr(&cont.lower.mask, sizeof(cont.lower.mask))];
        }
        else if (
            (fields & ICMPDECODE_LOWERMASK) ==
            ICMPDECODE_IP) /* the order of this if statement is significant, as we may have IDSEQ and IP, in which case IPSEQ comes first */
    ...
    case 5:
        if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ)
        {
            if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TSTAMP)
            {
                ...
                [ret addObject:names[8][0]];
                str = [[NSString alloc]
                    initWithFormat:@"%u", cont.lower.tstamp.orig];
                [ret addObject:str];
                [str release];
            }
        ...
        else
        {
            if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TSTAMP)
            {
                ...
                str = [[NSString alloc]
                    initWithFormat:@"%u", cont.lower.tstamp.recv];
                [ret addObject:str];
                [str release];
            }
            else if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TCP)
            { /* XXX TODO: Make TCPDecode able to decode 8 bytes */
                ...
                str = [[NSString alloc]
                    initWithFormat:@"%u",
                                   cont.lower.ipdata.payload.tcphdr.sport];
                [child addObject:str];
                [str release];
                [ret addChild:child];
                [child release];
                ...
                str = [[NSString alloc]
                    initWithFormat:@"%u",
                                   cont.lower.ipdata.payload.tcphdr.dport];
                [child addObject:str];
                [str release];
                [ret addChild:child];
                [child release];
                ...
                str = [[NSString alloc]
                    initWithFormat:@"%u", cont.lower.ipdata.payload.tcphdr.seq];
                [child addObject:str];
                [str release];
                [ret addChild:child];
                [child release];
    ...
    case 6:
        ...
        else if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TSTAMP)
        {
            ...
            str = [[NSString alloc]
                initWithFormat:@"%u", cont.lower.tstamp.trans];
            [ret addObject:str];
            [str release];
        }
        break;
    ...
    case 7:
        if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ &&
            (fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_TSTAMP)
        {
            ret = [[OutlineViewItem alloc] init];
            [ret addObject:names[10][0]];
            str = [[NSString alloc]
...
- (uint16_t)computedChecksum
{
    ...
    if ([data length] < skip_bytes ||
        [data length] - skip_bytes < ICMPDECODE_HDR_MIN)
        return 0;
...
- (void)decodeIPData:(NSData*)dataVal
{
    ...
    subip = [[NSData alloc]
        initWithBytesNoCopy:(uint8_t*)[dataVal bytes] + ICMP_MINLEN
                     length:[dataVal length] - ICMP_MINLEN
               freeWhenDone:NO];

    if ((cont.lower.ipdata.ipdec =
             [[IPV4Decode alloc] initWithData:subip parent:self]) != nil)
    {
        ...
        if ([cont.lower.ipdata.ipdec nextLayer] == [UDPDecode class])
        {
            ...
            if ((subudp = [[NSData alloc]
                     initWithBytesNoCopy:(uint8_t*)[subip bytes] +
                                         [cont.lower.ipdata.ipdec frontSize]
                                  length:[subip length] -
                                         [cont.lower.ipdata.ipdec frontSize]
                            freeWhenDone:NO]) == nil)
            {
            ...
            if ((cont.lower.ipdata.payload.udpdec =
                     [[UDPDecode alloc] initWithData:subudp
                                              parent:self]) != nil)
            {
            ...
            [cont.lower.ipdata.ipdec nextLayer] == [TCPDecode class] &&
            ([subip length] - [cont.lower.ipdata.ipdec frontSize]) >= 8)
        {
            cont.lower.ipdata.payload.tcphdr = *(
                struct
                minitcp*)((uint8_t*)[subip bytes] + [cont.lower.ipdata.ipdec frontSize]);
...
- (void)encodeWithCoder:(NSCoder*)coder
{
    ...
    if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ)
        [coder encodeValueOfObjCType:@encode(struct idseq)
                                  at:&cont.upper.idseq];
    else if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_GWADDR)
        [coder encodeValueOfObjCType:@encode(struct in_addr)
                                  at:&cont.upper.gateway];
    else if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_PARAMPTR)
    ...
    if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_SUBMASK)
        ...
        [coder encodeValueOfObjCType:@encode(struct tstamp)
                                  at:&cont.lower.tstamp];
    ...
    {
        [coder encodeObject:cont.lower.ipdata.ipdec];
        [coder encodeValueOfObjCType:@encode(struct minitcp)
                                  at:&cont.lower.ipdata.payload.tcphdr];
    }
}
...
- (id)initWithCoder:(NSCoder*)coder
{
    if ((self = [super init]) != nil)
    {
        ...
        if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_IDSEQ)
            [coder decodeValueOfObjCType:@encode(struct idseq)
                                      at:&cont.upper.idseq];
        else if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_GWADDR)
            [coder decodeValueOfObjCType:@encode(struct in_addr)
                                      at:&cont.upper.gateway];
        else if ((fields & ICMPDECODE_UPPERMASK) == ICMPDECODE_PARAMPTR)
        ...
        if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_SUBMASK)
            ...
            [coder decodeValueOfObjCType:@encode(struct tstamp)
                                      at:&cont.lower.tstamp];
        else if ((fields & ICMPDECODE_LOWERMASK) == ICMPDECODE_IP)
        ...
        {
            cont.lower.ipdata.ipdec = [[coder decodeObject] retain];
            [coder decodeValueOfObjCType:@encode(struct minitcp)
                                      at:&cont.lower.ipdata.payload.tcphdr];
        }
        calced_sum = 0;
        frontSize = 0;
...
- (NSArray*)decoders
{
    ...
    if (fields & ICMPDECODE_IP && fields & ICMPDECODE_UDP)
        return [NSArray arrayWithObjects:cont.lower.ipdata.ipdec,
                                         cont.lower.ipdata.payload.udpdec,
                                         nil];
    return nil;
}
